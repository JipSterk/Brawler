...
namespace Brawler.UI
{
    public class InterfaceManager : MonoBehaviour
    {
        ...
        [SerializeField] private Text _namePlayerProfileOutputText;
        ...
        private void Start()
        {
            ...
            _newPlayerProfileButton.onClick.AddListener(() => _gameManager.UpdateMenuState(MenuState.NewPlayerInputProfile));
            _newProfileNameInputField.onEndEdit.AddListener(NamePlayerProfile);
            _quitButton.onClick.AddListener(Quit);

            _characterSelectElements = ListExtensions.FindObjectsOfTypeAll<CharacterItemUiElement>();

            var uiButtons = ListExtensions.FindObjectsOfTypeAll<UiButton>().ToArray();
            Action<MenuState> callback = _gameManager.UpdateMenuState;
        ...
        private void UpdateMenuUi(MenuState menuState)
        {
            ...
            switch (menuState)
            {
                ...
                case MenuState.OfflineMultiPlayer:
                    break;
                case MenuState.InGamePauseMenu:
                    break;
                case MenuState.PlayerInput:
                    //var playerControlsProfiles = _playerControlsManager.PlayerControlsProfiles;
        ...
        private void NamePlayerProfile(string profileName)
        {
            ...
            {
                _namePlayerProfileOutputText.text = "Please fill in a ProfileName.";
                _newProfileNameInputField.text = null;
                return;
            }
            //if (_playerControlsManager.DoesProfileNameExist(profileName))
            //{
            //    _namePlayerProfileOutputText.text = "This name is taken.";
            //    _newProfileNameInputField.text = null;
            //    return;
            //}

            //_playerControlsManager.CreatePlayerControlsProfile(profileName);
        }
    }
}